...
docker bulld -t tmcmillla/multi-worker:latest -t tmcmilla/multi-worker:$SHA -f ./worker/Dockerfile ./worker
...
kubectl set image deployments/client-deployment server=tmcmilla/multi-client:$SHA
